# Product Requirements Document — sizbiz
...
- Публичное название продукта: `sizbiz`
- Историческое внутреннее имя реализации: `topdim`
...
Основные возможности в реализованном MVP:

- вход в партнерский дашборд через web (`admin-app` на порту 3001) под ролью `PARTNER`;
- просмотр своей статистики и списка купонов;
- создание заявки на купон (статус LEAD) прямо из дашборда;
- получение уведомлений и согласование купона (approve / revision) через Telegram-бота;
- управление своими магазинами через `bazaar-service` partner API.

### Рекомендация

- Дальнейшее развитие кабинета (редактирование купонов в статусе ACTIVE, расширенная статистика, управление отзывами) вынести в следующие итерации.
...
- Dashboard
- Контент / Купоны
- Support: возвраты, жалобы, отзывы
- Справочники: мерчанты, категории
- Заказы и поиск купленного купона
- Пользователи и партнёрские заявки
- Система / Staff / Audit

### Рекомендация

Support, orders, reviews, complaints и users входят в текущий admin MVP. UI
базаров/магазинов и промокодов остаётся отдельным roadmap-модулем до подтверждения
product ownership и launch scope.
...
Для MVP этот контур не требует доступа партнёра в admin-app: создание красивой
карточки остаётся на модераторах, а статусы, согласование и погашение доступны в
partner-app и Telegram Bot.
...
sizbiz уже содержит значимый продуктовый фундамент, но он сформирован из
нескольких пересекающихся контуров: consumer coupons, directory/map, staff
moderation и merchant bot-flow.

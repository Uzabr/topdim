...
| Безопасная загрузка изображений | Partner UI предлагает `image/*`, но media-service пока не проверяет MIME/signature/размер на сервере; до production нужен отдельный hardening |
| Проверка нового исполнителя | `reassign` проверяет положительный ID, но не подтверждает в identity-service, что пользователь существует, активен и имеет staff role |

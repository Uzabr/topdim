...
| Выбор нового исполнителя | `reassign` server-side проверяет пользователя через identity-service; в admin UI пока остаётся ручной ввод ID вместо staff selector |

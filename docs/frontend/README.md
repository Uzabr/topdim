...
- В admin detail изменений компании показываются текущий снимок, field/location
  diff и полная хронологическая лента переходов с исполнителем и комментарием.
- Admin detail до решения показывает филиалы, которые нельзя отключить из-за
  активных кассиров, и блокирует approve. Backend повторяет проверку при approve.
  Предложения не считаются по филиалам, потому что текущая модель не хранит такую
  привязку.

...
describe('CouponActionMenu', () => {
  ...
  it('renders a support approval 409 inside the still-open modal without retrying', async () => {
    const user = userEvent.setup();
    mockedPatch.mockRejectedValueOnce({
      response: { status: 409, data: { message: 'Партнёр уже изменил статус купона' } },
    });
    renderMenu('WAITING_FOR_MERCHANT', 'ADMIN', 7);
    ...
    const dialog = await screen.findByRole('dialog', { name: 'Служебное решение' });
    await waitFor(() => expect(
      within(dialog).getByText('Партнёр уже изменил статус купона'),
    ).toBeTruthy());
    expect(mockedPatch).toHaveBeenCalledOnce();
    expect(screen.getByRole('dialog', { name: 'Служебное решение' })).toBeTruthy();
  });

  it.each([
    [403, 'Недостаточно прав для выполнения действия'],
    [404, 'Купон удалён или ссылка устарела'],
  ])('renders an archive %s failure inside the still-open modal', async (status, expectedMessage) => {
    const user = userEvent.setup();
    mockedPost.mockRejectedValueOnce({ response: { status } });
    renderMenu('PAUSED', 'ADMIN', 7);

    await user.click(screen.getByRole('button', { name: 'Действия' }));
    await user.click(screen.getByRole('menuitem', { name: 'Архивировать' }));
    await user.type(screen.getByRole('textbox', { name: 'Причина архивации' }), 'Снято по SUP-43');
    await user.click(screen.getByRole('button', { name: 'Архивировать' }));

    const dialog = await screen.findByRole('dialog', { name: 'Архивировать купон' });
    await waitFor(() => expect(within(dialog).getByText(expectedMessage)).toBeTruthy());
    expect(mockedPost).toHaveBeenCalledOnce();
    expect(screen.getByRole('dialog', { name: 'Архивировать купон' })).toBeTruthy();
  });
});

...
Весь UI-текст хранится в `src/locales/ru.json` по блокам (`nav`, `home`, `catalog`, `profile`, `cart`, …).

- В компонентах: `const { t } = useTranslation()` → `{t('profile.tabs.orders')}`
- В Zustand-сторах: `import i18n from '../i18n'` → `i18n.t('favorites.limit.auth')`
- В Zod-схемах: создавать схему в `useMemo(() => ..., [t])` внутри компонента
- Ссылки: только через `useLocalePath()` → `lp('/coupons')`

Перевод на узбекский: отредактировать `src/locales/uz.json` (сейчас копия `ru.json`):

```bash
node scripts/generate-uz-from-ru.mjs   # сбросить uz из ru
```

Не переводится через i18n: контент с API (названия купонов), demo-данные в `DEMO_*` / `data/topdim.ts`.

### 6. Интерактивная карта (2GIS)
